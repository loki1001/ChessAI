...
public class Board extends JPanel implements MouseListener, MouseMotionListener {
    ...
    private Pair<Integer, Pair<Piece, Square>> MinMax_SelectPiece(boolean turnSelector, int depthLevel, int alpha, int beta, String prevPos, Stack<String> futureMoves) {
        LinkedList<Piece> pieces = null;
        ...
        for (int i = 0; i < pieces.size(); i++) {
                    ...
                    (!turnSelector && (pieceTemp.getColor() == 0))))
            {
                ...
                Pair<Integer, Square> r =
                        MinMax_SelectSquare(pieceTemp, turnSelector, depthLevel + 1, alpha, beta,
                ...
                if ((turnSelector) && ((oppSq == null) || (valMinMax < oppMinMaxVal))) {
                    ...
                    beta = Math.min(beta, oppMinMaxVal);
                    if (alpha >= beta) {
                    	break;
                    }
                }
                if ((!turnSelector) && ((oppSq == null) || (valMinMax > oppMinMaxVal))) {
                    ...
                    bestFutureMoves.removeAllElements();
                    bestFutureMoves.addAll(tempFutureMoves);
                    alpha = Math.max(alpha, oppMinMaxVal);
                    if (alpha >= beta) {
                    	break;
                    }
                }
    ...
    private Pair<Integer, Square> MinMax_SelectSquare(Piece chessPiece, boolean turnSelector, int depthLevel, int alpha, int beta, String prevPos, Stack<String> futureMoves) {
        ...
        for (int j = 0; j < possibleMoves.size(); j++) {
                    ...
                    )
            {
                ...
                Pair<Integer, Pair<Piece, Square>> r = MinMax_SelectPiece(
                        !turnSelector, depthLevel, alpha, beta, prevPos, tempFutureMoves);
                valMinMax = r.getKey();
    ...
    private boolean EvadeCheck() {
        ...
        Pair<Integer, Square> r = MinMax_SelectSquare(Bk, false, 0, Integer.MIN_VALUE, Integer.MAX_VALUE, Bk.getPositionName() + "\r\n", tempFutureMoves);
        int valMinMax = r.getKey();
                ...
                if (legalMoves.contains(sq) && movable.contains(sq)
                    ...
                    if (cmd.blackCheckMated()) {
                        ...
                        if (!whiteTurn) {
                            ...
                            Pair<Integer, Pair<Piece, Square>> r = MinMax_SelectPiece(false, 0, Integer.MIN_VALUE, Integer.MAX_VALUE, newText, futureMoves);
